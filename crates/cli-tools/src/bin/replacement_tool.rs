...
#[derive(Parser, Debug)]
#[command(about = "将目标json中不兼容特定编码的字符映射为特定编码的替身字符的实用工具")]
pub struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
pub enum Commands {
    ...
    Map {
        ...
        /// 输出目录的路径
        #[arg(long, default_value = "./replaced/")]
        output: String,

        /// 包含替身字符的 JSON 文件路径
        #[arg(long, default_value = "replacement_pool.json")]
...
fn map(path: Vec<String>, output: String, replacement_pool: String) -> Result<()> {
    let files = cli_tools::collect_files(path, Some("json"))?;
    ...
    for file in files {
        ...
        let out_path =
            cli_tools::to_dir(&output, &file).ok_or_else(|| anyhow!("构建输出路径失败: {file}"))?;
        new_text.write_to_path(out_path)?;
    }

    replacement_pool.write_mapping_to_path(
        cli_tools::to_dir(&output, "mapping.json").ok_or_else(|| anyhow!("构建输出路径失败"))?,
    )?;
...
fn generate_pool(path: Vec<String>, output: String, encoding: EncodingType) -> Result<()> {
    println!("开始生成 {encoding} 替身池...");

    let mut pool = PoolBuilder::new(encoding);
    pool.generate_pool()?;
    ...
    println!("成功生成替身字符池，包含 {pool_len} 个字符");
    println!("保存到: {output}");
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::GeneratePool {
            ...
            output,
            encoding,
        } => generate_pool(path, output, encoding)?,
    }
